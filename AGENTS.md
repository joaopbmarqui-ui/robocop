...
- **Driving the TUI for tests.** It is a full-screen interactive Textual app; demo it through a desktop terminal + computer use. For deterministic, scripted launches the app honors `DISPATCH_TEST_PREFILL=<path-to-json>` to open the New Job form pre-filled. The full test suite (`/workspace/.venv/bin/python -m pytest`) takes ~100s here.
- **Two expected test failures in the cloud checkout.** `tests/test_install_onboarding.py::test_install_creates_runtime_artifacts_with_mocked_edge_tools[*]` assert the repo path contains `robocop`. CI checks out to a `robocop`-named path, but the cloud workspace is `/workspace`, so those two cases fail here on `assert "robocop" in launcher_text` while the other ~474 tests pass. This is an environment/path artifact, not a code regression.
