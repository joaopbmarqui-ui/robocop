...
def test_iter_table_sizes_isolates_recoverable_failures(monkeypatch, caplog) -> None:
    """ValueError and RuntimeError must not abort later tables in the size batch."""
    import logging
    ...
    async def fake_run_impala_shell(statement: str) -> str:
        ...
        if "`boom`" in statement or statement.endswith("boom;"):
            raise RuntimeError("impala-shell exited 1: AnalysisException")
...
def test_iter_table_sizes_propagates_oserror(monkeypatch) -> None:
    impala.reset_query_ledger_for_tests()
    monkeypatch.setattr(impala, "external_running_query_count", lambda: 0)
...
def test_iter_table_sizes_propagates_typeerror(monkeypatch) -> None:
    impala.reset_query_ledger_for_tests()
    monkeypatch.setattr(impala, "external_running_query_count", lambda: 0)
...
def test_iter_table_sizes_propagates_cancellederror(monkeypatch) -> None:
    impala.reset_query_ledger_for_tests()
    monkeypatch.setattr(impala, "external_running_query_count", lambda: 0)
